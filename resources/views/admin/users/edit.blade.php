        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" placeholder="e.g. john@example.com"
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-150 space-y-4">
                <h3 class="text-sm font-bold text-gray-800">Change Password (Optional)</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="password" name="password" id="password" placeholder="Min 8 characters"
                               class="mt-2 block w-full bg-white rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('password') border-red-300 focus:ring-red-500 focus:border-red-500 @enderror">
                        @error('password')
                            <p class="mt-1.5 text-xs text-red-650">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password_confirmation" id="password_confirmation" placeholder="Retype password"
                               class="mt-2 block w-full bg-white rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="bio" id="bio" rows="4" placeholder="Brief details shown on author profile pages..."
                          class="mt-2 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('bio', $user->bio) }}</textarea>
                @error('bio')
                    <p class="mt-1.5 text-xs text-red-650">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-4 mt-2">
                    @if($user->avatar)
                        <img src="{{ asset('storage/' . $user->avatar) }}" class="w-16 h-16 object-cover rounded-full border border-gray-200" alt="">
                    @endif
