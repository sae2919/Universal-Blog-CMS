    ...
    <div class="bg-white rounded-xl border border-gray-150 shadow-sm p-6">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" placeholder="e.g. Technology"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" placeholder="Brief description of the category..."
                          class="mt-2 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs text-red-650">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="mt-2 flex items-center gap-3">
                        <input type="color" name="accent_color" id="accent_color" value="{{ old('accent_color', $category->accent_color ?? '#4f46e5') }}"
                ...
                <div>
                    ...
                    <input type="text" name="icon_emoji" id="icon_emoji" value="{{ old('icon_emoji', $category->icon_emoji) }}" placeholder="e.g. 💻, ⚽, 📚"
                           class="mt-2 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('icon_emoji')
                        <p class="mt-1.5 text-xs text-red-650">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <div class="flex items-center gap-4 mt-2">
                    @if($category->image)
                        <img src="{{ asset('storage/' . $category->image) }}" class="w-16 h-16 object-cover rounded-lg border border-gray-200" alt="">
                    @endif
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
                <a href="{{ route('admin.categories.index') }}" class="px-4 py-2.5 text-sm font-medium text-gray-755 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-all">
                    Cancel
                </a>
