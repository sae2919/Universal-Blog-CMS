...
class AiTest extends TestCase
{
    ...
    public function test_generate_article_endpoint_autocompletes_successfully()
    {
        ...
        $this->assertEmpty($response->json('featured_image_url'));
        $this->assertEmpty($response->json('featured_image_path'));
        $this->assertIsArray($response->json('generated_media'));
        $this->assertEmpty($response->json('generated_media'));

        $content = $response->json('content');
        // Ensure placeholders were stripped/removed and no image divs remain
        $this->assertStringNotContainsString('tiptap-image-wrapper', $content);
        $this->assertStringNotContainsString('post-slider', $content);
